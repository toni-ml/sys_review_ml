...
tidy_best_model <- function(data, metric, rev_conf = 1:4, 
                            ti_tiab_conf = c("ti", "tiab"), 
                            arrange = FALSE, 
                            desc = TRUE) {
  data %>%
    filter(
      rev_id %in% rev_conf,
      ti_tiab %in% ti_tiab_conf
    ) %>%
    select(dat_id, !!sym(metric)) %>%
    {if (arrange == TRUE) {
      if(desc == TRUE){
...
store_eval_dat_pretrain <- function(i){
  #+++++++++++++++++++++++
  # load data
  #+++++++++++++++++++++++
  ...
  results_temp <- ls_model$fit$ens_model$resample %>%  
    left_join(
  ...
  ls_temp_return <- 
    list(
      ...
      model_type = as.character(ls_model$fit$ens_model$modelType),
      model_method = as.character(ls_model$fit$ens_model$method),
      
      best_tune = c(ls_model$fit$models$baselearner01$bestTune,ls_model$fit$models$baselearner02$bestTune, ls_model$fit$models$baselearner03$bestTune),
      
      tune_time = as.character(hms::as_hms(ls_model$fit$ens_model$times$everything[[3]])),
      
      optim_metric = as.character(ls_model$fit$ens_model$metric),
      ...
      # train_cm
      #+++++++++++++++++++++++
      train_brier_score = as.numeric(mean((ls_model$train_pred_prob - ifelse(ls_model$train_y_ref == lvls[2], 0, 1))^2)),
      ...
      train_brier_skill_score = as.numeric(1 - mean((ls_model$train_pred_prob - ifelse(ls_model$train_y_ref == lvls[2], 0, 1))^2)/mean((ls_model_null$train_pred_prob[["inclusion"]] - ifelse(ls_model_null$train_y_ref == lvls[2], 0, 1))^2)),
      train_brier_comp_score = as.numeric(mean((ls_model_null$train_pred_prob[["inclusion"]] - ifelse(ls_model_null$train_y_ref == lvls[2], 0, 1))^2) - mean((ls_model$train_pred_prob - ifelse(ls_model$train_y_ref == lvls[2], 0, 1))^2)),
      train_auc =  as.numeric(ModelMetrics::auc(actual = ifelse(ls_model$train_y_ref == lvls[2], 0, 1), predicted = ls_model$train_pred_prob)),
      train_cl_error = as.numeric(ModelMetrics::ce(actual = ifelse(ls_model$train_y_ref == lvls[2], 0, 1), predicted = ls_model$train_pred_prob)),
      train_entropy = as.numeric(ModelMetrics::logLoss(actual = ifelse(ls_model$train_y_ref == lvls[2], 0, 1), predicted = ls_model$train_pred_prob)),
      train_gini = as.numeric(MLmetrics::Gini(y_pred = ls_model$train_pred_prob, y_true = ifelse(ls_model$train_y_ref == lvls[1], 1, 0))),
      ...
      # test_cm
      #+++++++++++++++++++++++
      test_brier_score = as.numeric(mean((ls_model$test_pred_prob - ifelse(ls_model$test_y_ref == lvls[2], 0, 1))^2)),
      test_brier_score_ref = as.numeric(mean((ls_model_null$test_pred_prob[["inclusion"]] - ifelse(ls_model_null$test_y_ref == lvls[2], 0, 1))^2)),
      test_brier_skill_score = as.numeric(1 - mean((ls_model$test_pred_prob - ifelse(ls_model$test_y_ref == lvls[2], 0, 1))^2)/mean((ls_model_null$test_pred_prob[["inclusion"]] - ifelse(ls_model_null$test_y_ref == lvls[2], 0, 1))^2)),
      test_brier_comp_score = as.numeric(mean((ls_model_null$test_pred_prob[["inclusion"]] - ifelse(ls_model_null$test_y_ref == lvls[2], 0, 1))^2) - mean((ls_model$test_pred_prob - ifelse(ls_model$test_y_ref == lvls[2], 0, 1))^2)),
      test_auc = as.numeric(ModelMetrics::auc(actual = ifelse(ls_model$test_y_ref == lvls[2], 0, 1), predicted = ls_model$test_pred_prob)),
      test_cl_error = as.numeric(ModelMetrics::ce(actual = ifelse(ls_model$test_y_ref == lvls[2], 0, 1), predicted = ls_model$test_pred_prob)),
      test_entropy = as.numeric(ModelMetrics::logLoss(actual = ifelse(ls_model$test_y_ref == lvls[2], 0, 1), predicted = ls_model$test_pred_prob)),
      test_gini = as.numeric(MLmetrics::Gini(y_pred = ls_model$test_pred_prob, y_true = ifelse(ls_model$test_y_ref == lvls[1], 1, 0))),
...
#***************************************************************************************************************************************************************
#**********************         store_eval_dat - softvoting         #*******************************************************************************************
#***************************************************************************************************************************************************************


store_eval_dat_pretrain_softvoting <- function(i){
  #+++++++++++++++++++++++
  # load data
  #+++++++++++++++++++++++
  fit_name <- paste0(ml_model, "_", grid_f$data_id_config[i])
