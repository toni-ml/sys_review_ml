...
eval_list <- foreach(i = 1:nrow(grid_f), 
                     ...
                     .export = c("times")) %dopar% {
                       if(str_detect(ml_model, "softvoting")){
                         store_eval_dat_pretrain_softvoting(i)
                       } else {
                         store_eval_dat_pretrain(i)
                       }
...
saveRDS(eval_dat %>% as_tibble(), paste0("02outputs/04ensemble/",ml_model_path, "/eval_",ml_model, ".RDS"))
